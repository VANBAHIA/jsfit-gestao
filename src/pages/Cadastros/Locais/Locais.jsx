...
import { MapPin, Search, Loader, Edit, Trash2, Plus, Filter, X } from 'lucide-react';
...
function Locais() {
  ...
  const handleEditarLocal = async (local) => {
    try {
      const resposta = await locaisService.buscarPorId(local.id);
      setLocalSelecionado(resposta.data?.data || resposta.data);
      setMostrarForm(true);
    } catch (error) {
      alert('Erro ao carregar dados do local: ' + error.message);
    }
  };

  const handleSalvarLocal = async (dados) => {
    try {
      setSalvando(true);

      if (localSelecionado) {
        await locaisService.atualizar(localSelecionado.id, dados);
      } else {
  ...
  const handleConfirmarExclusao = (local) => {
    setConfirmDelete({ isOpen: true, local });
  };

  const handleExcluirLocal = async () => {
    try {
      await locaisService.excluir(confirmDelete.local.id);
      setConfirmDelete({ isOpen: false, local: null });
      carregarDados();
    } catch (error) {
  ...
  const locaisFiltrados = locais.filter(local => {
    ...
    const matchBusca = !filtros.busca || local.nome?.toLowerCase().includes(busca);
    const matchStatus = !filtros.status || local.status === filtros.status;
  ...
  return (
    <div className="p-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="p-6 border-b flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">Gestão de Locais</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase">Nome do Local</th>
        ...
        {locaisFiltrados.length === 0 && !erro && (
          <div className="p-12 text-center">
            ...
            <p className="text-sm text-gray-500">
              {!filtros.busca && !filtros.status && 'Clique em "Novo Local" para começar'}
            </p>
          </div>
        )}
      </div>
      ...
      <ConfirmDialog
        ...
        mensagem={`Tem certeza que deseja excluir o local "${confirmDelete.local?.nome}"? Esta ação não poderá ser desfeita.`}
        onConfirmar={handleExcluirLocal}
        onCancelar={() => setConfirmDelete({ isOpen: false, local: null })}
      />
    </div>
  );
}
